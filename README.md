...
```bash
docker build -t playwright-fakestoreapi-tests .
```
...
```bash
docker run --rm playwright-fakestoreapi-tests
```
...
```bash
docker run --rm -v ${PWD}/playwright-report:/app/playwright-report playwright-fakestoreapi-tests
```
